...
class ClientProductController extends Controller
{
        ...
        try {
            ...
            if (!$request->has_variants) {
                StockMovement::create([
                    ...
                    'notes' => 'Initial product stock upload'
                ]);
            }
    ...
    {
        try {
            $product = Product::with('client','productImages','variants','category', 'brand', 'country','county','city')->find($id);
    ...
    public function delete(Request $request)
    {
        ...
        DB::beginTransaction();
        try {
            $product = Product::where('id', $product_id)->where('client_id', $client_id)->firstOrFail();
            ...
            DB::rollBack();
            ActivityLogger::log('item_delete_failed', 'Item not found.', $request, 'products');
            return response()->json([
                'status' => 'failed',
                'message' => 'Product not found',
                'error' => $e->getMessage()
