  ...
  try {
    ...
    if (error.response && error.response.status === 422) {
      Object.entries(error.response.data.errors).forEach(([field, messages]) => {
        document.getElementById(`${field}-error`).innerText = messages[0];
      });
    } else {
...
document.addEventListener("click", function (event) {
  if (event.target.classList.contains("add-variant")) {
    let newRow = document.querySelector(".variant-row").cloneNode(true);
    newRow.querySelectorAll("input").forEach(input => input.value = "");

    ["color", "size", "qty2"].forEach(field => {
      let inputField = newRow.querySelector(`input[name="${field}[]"]`);
      if (!inputField.nextElementSibling?.classList.contains(`${field}-error`)) {
    ...
    if (!newRow.querySelector(".remove-variant")) {
      let removeBtn = document.createElement("button");
      removeBtn.type = "button";
      removeBtn.classList.add("btn", "btn-danger", "remove-variant");
