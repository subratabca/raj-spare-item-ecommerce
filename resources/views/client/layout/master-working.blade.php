...
document.addEventListener("DOMContentLoaded", async function () {
  ...
  try {
      ...
      if (response.status === 200) {
        ...
        document.getElementById('docImg1').src = userData['doc_image1'] ? "/upload/client-document/small/" + userData['doc_image1'] : "/upload/no_image.jpg";

        document.getElementById('docImg2').src = userData['doc_image2'] ? "/upload/client-document/small/" + userData['doc_image2'] : "/upload/no_image.jpg";

        //End client-document.blade.php
        ...
        // Attach event listener to update districts when the division changes
        ...
        // Attach event listener to update cities when county changes
    ...
    function getNotificationLink(notification) {
        if (notification.data) {
            if (notification.data.order_id) {
                ...
                return `/client/food/details/${notification.data.food_id}?notification_id=${notification.id}`;
...
async function loadCounties(countryId, selectedCountyId = null) {
    try {
...
async function loadCities(countyId, selectedCityId = null) {
    try {
