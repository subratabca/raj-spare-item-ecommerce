  ...
  <div class="modal-dialog modal-lg" role="document">
    <div class="modal-content">
      ...
      <div class="modal-footer">
        ...
        <button type="button" class="btn btn-success" id="send-reply-btn">Send Reply</button>
        <button type="button" class="btn btn-danger" id="send-investigation-btn">Further Investigation</button>
      </div>
    </div>
  </div>
</div>
...
<script>
  ...
  document.addEventListener("DOMContentLoaded", function() {
    ...
    document.getElementById('send-investigation-btn').addEventListener('click', function(event) {
      event.preventDefault();
      sendInvestigation();
    });
    ...
    $('#reply-modal').on('hidden.bs.modal', async function () {
      ...
      if (uploadedImageUrl) {
        try {
          await axios.post('/admin/delete-editor-image', { image_url: uploadedImageUrl });
    ...
    input.onchange = async () => {
      ...
      if (file) {
        ...
        try {
          ...
          const res = await axios.post('/admin/upload-editor-image', formData, {
            headers: {
  ...
  async function sendReply() {
    ...
    if (textMessage === '') {
      ...
      try {
        let res = await axios.post("/admin/complain-solved", formData, config);
        if (res.status === 201) {
  ...
  async function sendInvestigation() {
    ...
    if (textMessage === '') {
      ...
      try {
        let res = await axios.post("/admin/complain-investigation", formData, config);
        if (res.status === 201) {
          successToast(res.data.message || 'Complain investigation done successfully');
          uploadedImageUrl = null;
          uploadedImageIndex = null;
          window.location.href = '/admin/complain-list';
          document.getElementById('save-form').reset();
