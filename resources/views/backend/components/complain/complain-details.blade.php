        ...
        <div class="card mb-4 border-2 border-primary">
            <div class="card-body">
                      ...
                      <table class="table table-hover">
                        <thead class="table-light">
                          <tr>
                            ...
                            <th class="text-truncate">Reply By</th>
                            <th class="text-truncate">Reply Message</th>
                ...
                <div class="text-center">
                    <button type="button" onclick="complainSend()" id="send-complain" class="btn btn-primary me-3">Send to client</button>
                    <button type="button" onclick="complainSolved()" id="complain-solved" class="btn btn-success me-3">Solved</button>
                </div>
            </div>
        </div>
    </div>
...
<script>
    ...
    async function ComplainDetailsInfo() {
        ...
        try {
            ...
            let res = await axios.get("/admin/complain/details/info/" + id);
            ...
            document.getElementById('send-complain').disabled = status !== 'pending';
            document.getElementById('complain-solved').disabled = status !== 'under-review';
    ...
    async function complainSend() {
        showLoader();
        try {
          let id = document.getElementById('complainID').value;
          let res = await axios.get("/admin/complain-send/" + id);

          if (res.status === 200) {
            successToast(res.data.message || "Complain sent successfully.");
            window.location.href = '/admin/complain-list';
          } else {
            errorToast("Request failed");
          }
        } catch (error) {
